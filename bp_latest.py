...
# json_fp = 'D://Json-pdf//updated-billing-2.json'
# pdf_fp = 'D://Json-pdf//Output files//Billing-oct-28-23.pdf'
...
def jsonTOpdf(json_data, billing_date):
    ...
    for service in data["billing_group"]["Charges_by_service"]:
        ...
        if 'marketplace' in service:
            ...
            # marketplace_charges.append(market_data)
    # print(marketplace_charges)
            for charge in market_data:
                y_position = check_page_space(y_position) 
                
                c.setFont("AmazonEmberBold", 9)
                marketplace_name = charge['service_name']
                marketplace_total = f"USD {charge['service_total']:.2f}"
                
                c.drawString(30, y_position, marketplace_name)
                c.drawString(width - c.stringWidth(marketplace_total) - 30, y_position, marketplace_total)
                ...
                if 'Global' in charge:
                    for global_item in charge['Global']:
                        for item in global_item['line_items']:
                            ...
                            customized_entry_str = str(item.get('line_item_usage_type_customized', ''))
                           # c.drawString(40, y_position, customized_entry_str)
                            ...
                            c.setFillColorRGB(0.5, 0.5, 0.5)
                            #c.drawString(48, y_position, str(item.get('line_item_line_item_description', '')))
                            c.setFont("AmazonEmberRegular", 7)
                ...
                if 'regions' in charge and charge['regions']:
