...
from bp_updated import jsonTOpdf
...
def get_last_month_date_range():
    ...
    # Get the year and month of the last month in 'YYYY-MM' format
    year_month_last = last_month.strftime('%Y-%m')

    # Format the dates
    formatted_date_range_last_month = f"{first_day_last_month.strftime('%b %d')} - {last_day_last_month.strftime('%b %d, %Y')}"

    return year_month_last, formatted_date_range_last_month

# Example usage:
# year_month, date_range = get_last_month_date_range()
# print("Year-Month for Last Month:", year_month)
# print("Date Range for Last Month:", date_range)
...
def find_highest_market(data):
    if 'marketplace' in data["billing_group"]["Charges_by_service"]:  
        services = data["billing_group"]["Charges_by_service"]["marketplace"]
        ...
        return highest_service_market, highest_region
...
def generate_json_data(input):
    ...
    #         df = pd.read_csv(r"701980022429.csv")
        ...
        market_place_highest = None
        marketplace_heights_total = None
        marketplace_heights_region = None
        marketplace_heights_region_total = None
        ...
        account_id = str(account_id[0]).strip()
        ...
        billing_date,formatted_date_range = get_last_month_date_range()
        # print(billing_date)
        ...
        if "AWS Marketplace" in aws_services:
            print("marketplace ")
            database = "athena"
            table = "regions_code_map"

            # SQL query to select data from the regions table
            region_query = f"SELECT * FROM {table}"

            # Execute the query in Athena using Data Wrangler
            region_df = wr.athena.read_sql_query(sql=region_query, database=database)
            region_to_location = dict(zip(region_df['regioncode'], region_df['location']))
            ...
            marketplace_data['location'] = marketplace_data['product_region'].map(region_to_location)
            ...
            for service in market_place_services:
                ...
                for region in service_data['location'].unique():
                    region_data = service_data[service_data['location'] == region]
        ...
        if i["account_type"] =="cur":
            ...
            service_provide_total = df[cost_column][(df["is_flagged"] == False) & (df["product_servicename"].notna()) & (df["product_servicename"] != '')].sum()              
        if i["account_type"] =="billing_group":        
            aws_services = df["product_servicename"][df["is_flagged"] == False].unique()
    ...
    #     # aws_services=["AmazonEC2"]
        ...
        for service in aws_services:
            servicedata = df[(df["product_servicename"] == service) & (df["is_flagged"] == False)]
            service_total = servicedata[cost_column].sum()
            # print(servicedata['product_location'].dtype)  # Check data type
            # print(servicedata['product_location'].unique()) 
            ...
            print(service,service_total,region_null)
            ...
            if service == "Amazon Elastic Compute Cloud":
                ...
                for pattern in patterns:
                    ...
                    if pattern == "BoxUsage" or pattern == "SpotUsage":
                        # print("inside")
                        ...
                        # marketoption = grouped_df["product_marketoption"].unique()
                        # print(marketoption)
                        product_operating_system = grouped_df["product_operating_system"].unique()
                        # print(product_operating_system)
                ...
                final_grouped_df.to_csv("ec2-data.csv")
                for region in final_grouped_df['product_location'].unique():
                    ...
                    print("patterns_in_region:",patterns_in_region)
                    for pattern in patterns_in_region:
                        if pattern == "BoxUsage" or pattern == "SpotUsage":
                            pattern_data = region_data[region_data['pattern'] == pattern]
                            product_operating_systems = pattern_data["product_operating_system"].unique()
                            
                            # print(product_operating_systems)
                            for product_operating_system in product_operating_systems:
                                description = "Amazon Elastic Compute Cloud running "  + (" Linux/UNIX" if product_operating_system == "Linux" else " Windows" if product_operating_system == "Windows" else f" {product_operating_system}")
                                product_operating_system_data = pattern_data[pattern_data["product_operating_system"]==product_operating_system]
                                product_operating_system_data_cost = product_operating_system_data[cost_column].sum()
                                # print(product_operating_system_data[["line_item_line_item_description","line_item_usage_type_customized","line_item_usage_amount_with_units", cost_column]].to_dict(orient='records'))
                                pattern_entry = {
                                            'pattern': description,
                                            'pattern_total': product_operating_system_data_cost,
                                            'line_items':  product_operating_system_data[["line_item_line_item_description","line_item_usage_type_customized","line_item_usage_amount_with_units", cost_column]].to_dict(orient='records')
                                        }
                        else:
                            pattern_data = region_data[region_data['pattern'] == pattern]
                            pattern_total = pattern_data[cost_column].sum()
            ...
            elif service == "AWS CodeCommit" or service == "Amazon Interactive Video Service" or service == "Amazon Route 53" and region_null>0:
                    # print("elif")
                    grouped_df = servicedata.groupby([
                    'line_item_line_item_description',  
                    'line_item_usage_type', 
            ...
            elif service =="AWS Data Transfer" and  region_null >0 :
        ...
        result = find_highest_market(final_json)
        if result is None:
            print("no marketplace data")
        else:
            # print(result)
            m_highest_service,m_highest_region = result
            market_place_highest = m_highest_service["service_name"]
            marketplace_heights_total = m_highest_service["service_total"]
            marketplace_heights_region = m_highest_region["region"]
            marketplace_heights_region_total = m_highest_region["region_total"]
    ...
    # Calculate the sum of False values
        if i["account_type"] == "billing_group":
            ...
            api_data = {"account_num":account_id,"billing_period":f"{billing_date}-01",
                        ...
                        "market_highest_service":market_place_highest,"market_highest_service_amount":marketplace_heights_total,"market_highest_region_amount":marketplace_heights_region_total,"market_highest_region_name":marketplace_heights_region}
            print(api_data)
