...
import json
from validations_json_with_units_cur_data_optimized_v5 import generate_json_data
import requests
...
def build_queries(line_item_usage_account_id, month, year, last_month_str):
    ...
    SELECT line_item_usage_account_id, line_item_line_item_description, line_item_product_code, 
           ...
           product_product_family, line_item_unblended_cost,product_operating_system,line_item_blended_cost, 
           pricing_unit,product_region,product_product_name,product_servicename,product_from_location, product_to_location, product_location 
    FROM aws_cur_2_0_billing_group_prod 
    ...
    SELECT line_item_usage_account_id, line_item_line_item_description, line_item_product_code, 
           ...
           line_item_usage_end_date, bill_billing_entity, line_item_usage_type, 
           line_item_line_item_type, product_servicecode, product_instance_type, 
           product_product_family,product_region,product_product_name, line_item_unblended_cost, line_item_blended_cost, 
...
def process_account_data():
    ...
    if response.status_code == 200:
        ...
        print("Failed to fetch data:", response.status_code, response.text)    
    account_ids = [{"bill_payer_account_id":"088415856281","account_id":"418774679984"}]
    ...
    for account in account_ids:
        ...
        try:
            ...
            fail_data = {
                "account_num": line_item_usage_account_id,
                "billing_period": last_month_str_with_date,
                "remarks": str(e)
            }
            
            # Post to failstatus API
            fail_url = "http://44.229.219.3:2525/post_bill_summary_failstatus"
            fail_response = requests.post(fail_url, json=fail_data)
            
            # Check the response for the failstatus post
            if fail_response.status_code == 200:
                print(f"Failure status posted for account {line_item_usage_account_id}")
            else:
                print("Failed to post failure status:", fail_response.status_code, fail_response.text)            
